...
import (
	...
	pkgtest "knative.dev/client/pkg/util/test"
)
...
func (kp *knPlugin) Run(args ...string) pkgtest.KnRunResult {
	if kp.install {
		err := kp.Install()
		if err != nil {
			fmt.Printf("error installing kn plugin: %s\n", err.Error())
			return pkgtest.KnRunResult{}
		}
		defer kp.Uninstall()
	}
...
// Kn object to run `kn`
func (kp *knPlugin) Kn() pkgtest.Kn {
	return kp.kn
}
...
func RunKnPlugin(namespace string, pluginName string, args []string) pkgtest.KnRunResult {
	pluginArgs := pluginArgs(pluginName)
	args = append(args, []string{"--namespace", namespace}...)
	argsWithPlugin := append(pluginArgs, args...)
	return pkgtest.RunKn(namespace, argsWithPlugin)
}
